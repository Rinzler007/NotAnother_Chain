...
def is_valid(request):
    if request.method == 'GET':
        ...
        if is_valid:
            response = {'message': 'Yaas Queen. The Blockchain is valid.'}
        else:
            response = {'message': 'Brrrruuuuhhhhhh, we have a problem. The Blockchain is not valid.'}
    return JsonResponse(response)
...
def connect_node(request): #New
    if request.method == 'POST':
        ...
        response = {'message': 'Yoooooo MF. All the nodes are now connected. The DonkeyCoin Blockchain now contains the following nodes:',
...
def replace_chain(request): #New
    if request.method == 'GET':
        ...
        if is_chain_replaced:
            ...
            response = {'message': 'Kachinggggg. The chain is the largest one.',
